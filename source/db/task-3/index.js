...
for (let i = 0; i < 100; i++) {
    ...
    const { insertedId } = db.customers.insertOne(user);
    ...
    for (let j = 0; j < randomNumber(1, 10); j++) {
        ...
        const cursor = db.orders.insertOne(order);
    }
}

const ordersCount = db.orders.countDocuments({});
const ordersSize = db.orders.stats().size / 1024;
const customersSize = db.customers.stats().size / 1024;

print(`Orders count: ${ordersCount}`);
print(`Size of customers collection: ${Math.ceil(customersSize)}Kb`);
print(`Size of orders collection: ${Math.ceil(ordersSize)}Kb`);
